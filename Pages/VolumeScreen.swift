                            ...
                            ZStack {
                                ...
                                VStack(spacing: 4) {
                                    ...
                                    Slider(value: Binding(
                                        get: { Double(createAdditionalZStacks[index].volume) },
                                        set: { createAdditionalZStacks[index].volume = Float($0) }
                                    ), in: -100...100, step: 1) { _ in
                                        if let player = createAdditionalZStacks[index].player {
                                            player.volume = mapVolume(Double(createAdditionalZStacks[index].volume))
                                        }
                                    }
                                    .frame(width: geometry.size.width * 0.7)
                                }
